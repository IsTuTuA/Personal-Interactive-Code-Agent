...
import Persona_Interactive_Code_Agent as mini_pkg
from Persona_Interactive_Code_Agent import (
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        ), patch("Persona_Interactive_Code_Agent.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_agent_uses_anthropic_provider_and_openai_key_fallback(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            ...
            "Persona_Interactive_Code_Agent.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("Persona_Interactive_Code_Agent.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("Persona_Interactive_Code_Agent.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(mini_pkg.__file__).as_posix().endswith("/Persona_Interactive_Code_Agent/__init__.py")
...
def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "Persona_Interactive_Code_Agent", "--help"],
        capture_output=True,
        text=True,
    )
