...
from Persona_Interactive_Code_Agent import FakeModelClient, MiniAgent, SessionStore, WorkspaceContext
from Persona_Interactive_Code_Agent import cli as mini_cli
from Persona_Interactive_Code_Agent.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        "Persona_Interactive_Code_Agent.cli.OllamaModelClient",
        DummyModelClient,
    ):
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        ...
        clear=True,
    ), patch("Persona_Interactive_Code_Agent.cli.OllamaModelClient", DummyModelClient):
        args = mini_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = mini_cli.build_agent(args)
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("Persona_Interactive_Code_Agent.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "Persona_Interactive_Code_Agent.runtime"

    with patch("Persona_Interactive_Code_Agent.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
