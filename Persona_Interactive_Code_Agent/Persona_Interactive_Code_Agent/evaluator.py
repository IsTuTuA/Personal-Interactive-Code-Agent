...
try:
    ...
    def ZoneInfo(timezone_name):
        if str(timezone_name) in {"Asia/Shanghai", "Asia/Chongqing", "Asia/Harbin", "Asia/Urumqi"}:
            return timezone(timedelta(hours=8), str(timezone_name))
        if str(timezone_name).upper() in {"UTC", "ETC/UTC", "Z"}:
            return timezone.utc
        return timezone.utc
